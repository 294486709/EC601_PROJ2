...
def CNN(x):
	...
	conv2 = tf.nn.relu(
		tf.nn.conv2d(conv1, weights['w_conv2'], strides=[1, 1, 1, 1], padding='VALID') + biases['b_conv2'])
	conv2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

	conv3 = tf.nn.relu(
		tf.nn.conv2d(conv2, weights['w_conv3'], strides=[1, 1, 1, 1], padding='SAME') + biases['b_conv3'])
	conv3 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

	conv4 = tf.nn.relu(
		tf.nn.conv2d(conv3, weights['w_conv4'], strides=[1, 1, 1, 1], padding='VALID') + biases['b_conv4'])
	conv4 = tf.nn.max_pool(conv4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

	conv5 = tf.nn.relu(
		tf.nn.conv2d(conv4, weights['w_conv5'], strides=[1, 1, 1, 1], padding='VALID') + biases['b_conv5'])
...
def shuffle_batch(xtrain, ytrain):
...
def train_nn(x, xtrain, ytrain, xtest, ytest, y):
	...
	with tf.Session() as sess:
		...
		for epoch in range(NUM_EPOCH):
			xtrain, ytrain = shuffle_batch(xtrain, ytrain)
			epoch_loss = 0
